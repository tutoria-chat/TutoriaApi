...
- [ ] Add Files endpoints (upload, delete, list by module)
- [ ] Add ModuleAccessTokens endpoints (generate, revoke, list)

### Auth API Endpoints
- [ ] Implement `/auth/login` (username/password → JWT)
- [ ] Implement `/auth/token` (client_credentials → JWT)
- [ ] Implement `/auth/register/student` (public or restricted)
- [ ] Implement `/auth/password-reset-request`
- [ ] Implement `/auth/password-reset`
- [ ] Implement `/auth/me` (GET - get current user info)
- [ ] Implement `/auth/me` (PUT - update own profile)
- [ ] Implement `/auth/me/password` (PUT - change own password with current password verification)
- [ ] Implement `/auth/refresh` (refresh token endpoint)
...
- [ ] Configure automated tests (when added)
